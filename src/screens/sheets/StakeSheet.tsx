...
export function StakeSheet() {
  ...
  return (
    <BottomSheet id="stake">
      <div className="font-serif text-xl text-forest mb-[18px]">Stake into Vault</div>

      <div className="bg-surface rounded-[14px] p-3 flex items-center gap-2.5 mb-4">
        <div className="text-[26px]">🌶️</div>
        <div>
          <div className="text-[13px] font-medium text-forest">CHILI-GH-SUBANG-Q2</div>
          <div className="text-[11px] text-stone">Greenhouse · West Java · Est. 18% APY · 90 days</div>
        </div>
      </div>

      <div className="mb-3.5">
        <label className="text-[11px] font-semibold text-stone uppercase tracking-wider mb-1.5 block">Stake Amount</label>
      ...
      <div className="bg-mist rounded-[14px] p-3.5 mb-4">
        ...
        <div className="flex justify-between mb-2 text-[13px]">
          <span className="text-stone">Est. Profit (18%)</span>
          <span className="font-medium text-sprout">+{fmt(profit)}</span>
        </div>
        <div className="h-px bg-forest/10 mb-2" />
        <div className="flex justify-between text-sm">
          <span className="font-semibold text-forest">Est. Total Return</span>
          <span className="font-semibold text-forest">{fmt(total)}</span>
        </div>
      </div>

      <div className="text-[11px] text-stone leading-relaxed mb-4">
        ⚠️ Funds locked for 90 days. Exit early via Secondary Market.
      </div>

      <button
        onClick={() => { closeSheet(); showToast('✓ Stake confirmed on Solana') }}
